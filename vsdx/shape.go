...
// joining them in document order.
...
// double-scale them.
//
// a regression case: pre-fix this was strings.Contains(f, "Width"), which had false
...
func (s *Shape) SetLineWeight(v float64) {
	...
	// the writer notes §4: Visio annotates LineWeight with U='PT' even
...
// row when needed.
//
// a regression case: when we create a NEW local Row that shadows a master Row, we
...
func (s *Shape) AddGeometry() *Geometry {
	...
	// the writer notes §3: Visio writes NoShow/NoSnap/NoQuickDrag plus
...
//
// a regression case minimum-viable dependency walker — one level deep, single shape.
// Does NOT handle:
...
// shape pick up the new master state. a regression case.
//
...
// MasterPageID matches the given master ID. a regression case — useful after mutating
...
// local cell exists. a regression case.
//
...
// where the operation is conceptually "revert", not "delete". a regression case.
...
// out of range. a regression case: compound master shapes carry multiple geometry
